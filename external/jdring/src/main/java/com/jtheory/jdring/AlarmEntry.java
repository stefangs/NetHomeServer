...
public class AlarmEntry implements Comparable, java.io.Serializable {
    ...
    throws PastDateException {
        ...
        setName(_name);
        minutes = copyOf(_minutes, _minutes.length);
        hours = copyOf(_hours, _hours.length);
        daysOfMonth = copyOf(_daysOfMonth, _daysOfMonth.length);
        months = copyOf(_months, _months.length);
        daysOfWeek = copyOf(_daysOfWeek, _daysOfWeek.length);
        year = _year;
        listener = _listener;
        isRepeating = (_year == -1);
        isRelative = false;
    ...
    public int hashCode() {
        return name.hashCode() + ((int)alarmTime) + (isRelative ? 1 : 0) + (isRepeating ? 1 : 0) + minutes.hashCode() +
                hours.hashCode() + daysOfMonth.hashCode() + months.hashCode() + daysOfWeek.hashCode();
    }
